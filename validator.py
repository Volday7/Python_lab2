...
class Validator:
    ...
    def check(self):
        ...
        pattern = "[А-яа-я]{1,}"
        if re.match(pattern, str(self.political_views)) is None:
            return 'political_views'
        if re.match(pattern, str(self.worldview)) is None:
            return 'worldview'
        pattern = "\у\л\.\ [А-Яа-я \-.0-9]{1,}[а-яА-Я0-9]{1,}"
...
parser.add_argument('-input', type=str, help='Input file for reading', dest="f_in")
parser.add_argument('-output', type=str, help='Output file for writing correct data', dest="f_out")
args = parser.parse_args()
input_file = File(args.f_in)
output_file = open(args.f_out, 'w')
dict_error = {"email": 0, "weight": 0, "snils": 0, "passport_series": 0, "occupation": 0, "work_experience": 0, "political_views": 0, "worldview": 0, "address": 0, "True": 0}
counter = 0
...
for i in tqdm(input_file.data):
    ...
    if f.check() == "True":
        ...
        output_file.write("passport_series: " + str(i["passport_series"]) + "\n")
        output_file.write("occupation: " + i["occupation"] + "\n")
        output_file.write("work_experience: " + str(i["work_experience"]) + "\n")
